 ...
 * Must be loaded LAST (after products.js, cart.js, admin.js).
 * Depends on: products.js, cart.js, admin.js
 */
...
MG.Utils = {
  ...
  async submitLead(payload) {
    ...
    if (!url) {
      console.warn('[MG] Google Apps Script URL not configured.');
    ...
    try {
      ...
      console.warn('[MG] submitLead error:', error);
...
MG.Checkout = {
  ...
  init() {
    ...
    /* Form submit → Google Sheets, then WhatsApp */
      ...
      ?.addEventListener('submit', async e => {
        ...
        if (msg) {
          ...
          msg.querySelector('p').textContent = result.ok
            ? 'Your order was submitted. Continue to WhatsApp to confirm availability and delivery.'
            : 'We could not confirm the Google Sheet submission. Continue to WhatsApp so your order is not lost.';
        }
      });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (path.includes('admin.html') && (!MG.Auth || MG.Auth.isLoggedIn())) {
    MG.Admin.init();
  }
});
